...
def strat1_ce(
    ...
    if (
        ...
        and (contract_historical_oi_data[-1])["oi"] > avg
    ):
        ma_calc = True
    ...
    if ma_calc == True:
        ...
        if (
            ...
            and list(historical_df["WMA (HMA,3)"])[-1] > 55
        ):
            closed_price = list(historical_df["close"])[-1]
            tradebook_strat1_ce = pd.read_excel("tradebook_strat1_ce.xlsx")
            new_df = pd.DataFrame(
                {
            ...
            tradebook_strat1_ce = pd.concat(
                [tradebook_strat1_ce, new_df], ignore_index=True
            )
            tradebook_strat1_ce.to_excel("tradebook_strat1_ce.xlsx", index=False)
